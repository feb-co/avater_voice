...
class AvatarRequestOutput:
    ...
    def add(self, next_output: "AvatarRequestOutput") -> None:
    ...
    def from_seq_group(
    ...
    ) -> Optional["AvatarRequestOutput"]:
        finished = seq_group.is_finished()

        if seq_group.llm_seq_group.request_id in seq_id_to_seq_group:
            group: SequenceGroupBase = seq_id_to_seq_group[seq_group.llm_seq_group.request_id]
        ...
        if llm_sampling_params is None:
            raise ValueError("llm Sampling parameters are missing for a CompletionRequest.")

        if llm_sampling_params.output_kind == RequestOutputKind.FINAL_ONLY and (not finished):
            return None

        # Init cache (if needed)
        if use_cache and seq_group.llm_seq_group.cached_request_output is None and seq_group.tts_seq_group.cached_request_output is None:
            seq_group.llm_seq_group.cached_request_output = AvatarRequestOutput(  # type: ignore
                request_id="",
                prompt=None,
                prompt_token_ids=[],
            ...
            seq_group.tts_seq_group.cached_request_output = AvatarRequestOutput(  # type: ignore
                request_id="",
                prompt=None,
                prompt_token_ids=[],
                prompt_logprobs=None,
...
class AvatarRequestOutputFactory:
    @staticmethod
    def create(
        seq_group: AvatarSequenceGroup,
        seq_id_to_seq_group: Dict[str, SequenceGroupBase],
        ...
        if seq_group.pooled_data is not None:
            ...
            return AvatarRequestOutput.from_seq_group(
                seq_group,
                use_cache,
                seq_id_to_seq_group
            )
