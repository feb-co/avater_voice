...
class AvatarMultiTokenSampler(nn.Module):
    ...
    def _init_sampling_tensors(
        ...
        """
        _, code_size, vocab_size = logits.shape
    ...
    def forward(
        ...
        * Encapsulate arguments required for deferred Pythonization
        ...
        seqlen, code_size, vocab_size = logits.shape
        ...
        logits = apply_min_tokens_penalty(logits, sampling_metadata)
        ...
        logits = logits.view(seqlen, -1).div(sampling_tensors.temperatures.unsqueeze(dim=1)).view(seqlen, code_size, vocab_size)
        ...
        if do_min_p:
            logits = apply_min_p(logits, sampling_tensors.min_ps)
        ...
        maybe_deferred_sample_results, maybe_sampled_tokens_tensor = sample(
            probs,
            logprobs,
            sampling_metadata,
            sampling_tensors,
...
def apply_min_tokens_penalty(
    ...
    if logits_to_penalize:
        ...
        row_indices, dim_indeices = tuple(zip(*logits_to_penalize))
        logits[row_indices, ..., dim_indeices] = -float("inf")
...
def apply_top_k_top_p(
    ...
    seqlen, code_size, vocab_size = logits.size()

    logits_sort, logits_idx = logits.sort(dim=-1, descending=False)

    # Apply top-k.
    top_ks = top_ks.view(-1, 1).expand(seqlen, code_size).to(torch.long)
    top_k_indices = (vocab_size - top_ks).unsqueeze(-1)
    batch_indices = torch.arange(seqlen, device=logits.device).view(-1, 1).expand(seqlen, code_size)
    code_indices = torch.arange(code_size, device=logits.device).view(1, -1).expand(seqlen, code_size)
    top_k_thresholds = logits_sort[batch_indices, code_indices, top_k_indices.squeeze(-1)]
    top_k_mask = logits_sort < top_k_thresholds.unsqueeze(-1)
    logits_sort.masked_fill_(top_k_mask, -float("inf"))

    # Apply top-p.
    probs_sort = logits_sort.softmax(dim=-1)
    probs_sum = probs_sort.cumsum(dim=-1)
    top_ps = top_ps.view(-1, 1).expand(seqlen, code_size)
    top_p_mask = probs_sum <= 1 - top_ps.unsqueeze(-1)
    # at least one
    top_p_mask[:, :, -1] = False
...
def apply_min_p(
    ...
    min_p = min_p.view(-1, 1).expand(seqlen, code_size).unsqueeze(-1)
    ...
    for sampling_type in SamplingType:
        ...
        if sampling_type == SamplingType.GREEDY:
            greedy_samples = torch.argmax(
                logprobs[long_sample_indices], dim=-1
            )
...
def _multinomial(
    ...
    # in a copy already.
    seqlen, code_size, d_prob = probs.size()

    if num_samples > 1:
        probs = probs.repeat_interleave(num_samples, dim=0)
    uniform_samples = torch.empty_like(probs)

    if seq_groups is None:
        uniform_samples.exponential_()
    else:
        sample_idx = 0
        for seq_group in seq_groups:
            seq_ids = seq_group.seq_ids
            ...
            uniform_samples[sample_idx:sample_idx + stride].exponential_(generator=seq_group.generator)
            sample_idx += stride

    return probs.div_(uniform_samples).argmax(dim=-1).view(-1, code_size, num_samples)
...
def get_logprobs(
    ...
    """
    seqlen, code_size, vocab_size = logprobs.size()
    ...
    for (seq_group, sample_result) in zip(sampling_metadata.seq_groups, sample_results):
        ...
        assert len(next_token_ids)//code_size == len(query_indices)
...
def build_sampler_output(
    ...
    if skip_sampler_cpu_output:
        ...
        assert len(sampling_metadata.seq_groups) == len(maybe_deferred_sample_results)
        deferred_sample_results_args = None

        for (seq_group, sample_result) in zip(sampling_metadata.seq_groups, maybe_deferred_sample_results,):
            seq_ids = seq_group.seq_ids
            next_token_ids, parent_ids = sample_result
            seq_outputs: List[SequenceOutput] = []
            for parent_id, next_token_id in zip(parent_ids, next_token_ids):
                seq_outputs.append(SequenceOutput(
                    parent_seq_id=seq_ids[parent_id],
                    output_token=next_token_id,
                    logprobs=None
                ))
            sampler_output.append(CompletionSequenceGroupOutput(
                samples=seq_outputs,
                prompt_logprobs=None
            ))
